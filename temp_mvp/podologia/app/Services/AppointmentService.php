...
class AppointmentService
{
    ...
    {
        if ($appointment->status === Appointment::STATUS_ATTENDED) {
            return $appointment;
        }

        DB::transaction(function () use ($appointment) {
            $appointment->update(['status' => Appointment::STATUS_ATTENDED]);

            // Calcular distribución de ingresos
            $professional   = $appointment->professional;
            $total          = $appointment->fee_at_booking;
            $commissionRate = $professional->commission_pct / 100;

            if ($professional->isAdmin()) {
                // Dueña: todo para ella
                $ownerShare    = $total;
                $employeeShare = 0;
            } else {
                // Empleada: comisión para ella, resto para dueña
                $employeeShare = round($total * $commissionRate, 2);
                $ownerShare    = round($total - $employeeShare, 2);
            }

            // Registrar ingreso histórico inmutable
            IncomeRecord::create([
                ...
                'recorded_date'  => now()->setTimezone(self::TIMEZONE)->toDateString(),
            ]);

            // Actualizar contador de visitas del paciente
            $appointment->patient()->increment('visit_count');
            $appointment->patient()->update([
                'last_visit_at' => now(),
            ]);
        });

        return $appointment->fresh(['incomeRecord', 'patient', 'professional']);
    }
    ...
    {
        // Buscar por teléfono (identificador principal)
