...
class AvailabilityService
{
        ...
        while ($current->copy()->addMinutes($durationMins)->lte($slotEnd)) {
            ...
            $slots[] = [
                ...
                'available' => !$this->isSlotTaken($current, $slotEndTime, $existingAppointments)
                               && $current->isFuture(), // no mostrar slots pasados
            ];
    ...
    {
        $startOfDay = $date->copy()->utc()->startOfDay();
        $endOfDay   = $date->copy()->utc()->endOfDay();

        // Ajustar a UTC para la query (internamente todo se guarda en UTC)
